...
log_collector.setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(message)s'))
...
# --------------------------------------------------
# --------------------------------------------------
# Updated Simple Session Handling
# --------------------------------------------------
def run_agent(user_text: str, amzn_trace_id: str = None) -> tuple:
    # Use the official AWS trace ID if present, otherwise fallback to standard uuid
    session_id = amzn_trace_id if amzn_trace_id else str(uuid.uuid4())
    logger.info(f"Session {session_id[:8]}: Starting execution chain")
    ...
    return final_state["final_answer"], start_time, end_time
...
def invocations():
    log_collector.records = []
    
    payload = request.get_json() or {}
    user_text = payload.get("input", {}).get("text", "")

    # CRITICAL: Extract the active runtime trace tracking header from AgentCore
    amzn_trace_id = request.headers.get("X-Amzn-Trace-Id") or payload.get("traceId")

    # Pass the ID directly into the agent run execution
    final_answer, start_time, end_time = run_agent(user_text, amzn_trace_id)
    collected_logs = log_collector.records

    return jsonify({
        "completion": final_answer,
        "stop_reason": "end_turn",
        "evaluationInput": {
            "startTime": str(start_time),  # Root level mapping for the evaluator
            "endTime": str(end_time),
            "traceId": str(amzn_trace_id)
        },
        "agent": {
